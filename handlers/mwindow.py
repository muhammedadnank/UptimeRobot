...
def register(app: Client):
    ...
    async def cmd_mwindow(client: Client, message: Message):
        ...
        sent    = await message.reply("⏳ Fetching maintenance windows…", quote=True)
    ...
    async def cmd_delmwindow(client: Client, message: Message):
        ...
        await message.reply(f"⚠️ Delete maintenance window `{wid}`?", reply_markup=markup, quote=True)
